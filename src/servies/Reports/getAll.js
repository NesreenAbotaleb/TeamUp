...
const GetAll = async (token, navigate, setReports, code_Comm) => {
    try {
        console.log('Fetching reports with token:', token);
        console.log('Community code:', code_Comm);
        ...
        const response = await axios.get(`${api}/report/getReports/professors/${code_Comm}`, {
            headers: { 
                Authorization: `${token}`,
